...
def client_visit_analysis(df):
    ...
    visits_per_client = df.groupby('patient_id')['service_document_id'].nunique().reset_index()
...
def client_retention_analysis(df):
    ...
    monthly_clients = df.groupby('year_month')['patient_id'].nunique().reset_index()
    ...
    for _, row in monthly_clients.iterrows():
        ym = row['year_month']
        month_clients = set(df[df['year_month'] == ym]['patient_id'].unique())
...
def service_mix_analysis(df):
    ...
    df_with_segments = pd.merge(df, visits_per_client[['client_id', 'segment']], 
                               left_on='patient_id', right_on='client_id')
    ...
    # Add value labels
    eda_style.value_label_bars(ax, fmt='{:,.0f} ₽', fontsize=14)
